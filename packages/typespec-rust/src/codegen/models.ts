...
function recursiveBuildDeserializeBody(indent: helpers.indentation, use: Use, ctx: stateCtx): string {
  ...
  switch (ctx.type.kind) {
    ...
    case 'offsetDateTime': {
      ...
      const dateParse = ctx.type.encoding === 'unix_time' ? 'OffsetDateTime::from_unix_timestamp' : `time::parse_${ctx.type.encoding}`;
...
function recursiveBuildSerializeBody(indent: helpers.indentation, use: Use, ctx: stateCtx): string {
  ...
  switch (ctx.type.kind) {
    ...
    case 'offsetDateTime': {
      ...
      content = ctx.type.encoding === 'unix_time' ? `${ctx.srcVar}.unix_timestamp()` : `time::to_${ctx.type.encoding}`;
      switch (ctx.caller) {
        case 'hashmap':
