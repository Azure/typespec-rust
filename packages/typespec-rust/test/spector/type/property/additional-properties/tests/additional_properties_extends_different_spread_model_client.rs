...
use spector_addlprops::{
    models::{DifferentSpreadModelDerived, ModelForRecord},
...
async fn get() {
    ...
    let resp = client
        .get_additional_properties_extends_different_spread_model_client()
        .get(None)
        .await
        .unwrap()
        .into_model()
    ...
    assert_eq!(resp.derived_prop.unwrap().state.as_deref(), Some("ok"));
