...
use spector_addlprops::{models::ExtendsStringAdditionalProperties, AdditionalPropertiesClient};
use std::collections::HashMap;

fn body() -> ExtendsStringAdditionalProperties {
...
async fn get() {
    ...
    let resp = client
        .get_additional_properties_extends_string_client()
        .get(None)
        .await
        .unwrap()
        .into_model()
    ...
    assert_eq!(
        resp.name.as_deref(),
        Some("ExtendsStringAdditionalProperties")
    );
