...
fn derived() -> IsUnknownAdditionalPropertiesDiscriminatedDerived {
...
async fn get() {
    ...
    let resp = client
        .get_additional_properties_is_unknown_discriminated_client()
        .get(None)
        .await
        .unwrap()
        .into_model()
        .unwrap();
    match resp {
        IsUnknownAdditionalPropertiesDiscriminated::IsUnknownAdditionalPropertiesDiscriminatedDerived(d) => {
