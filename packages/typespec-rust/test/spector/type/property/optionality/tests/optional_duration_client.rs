...
use spector_optionality::{models::DurationProperty, OptionalClient};

#[tokio::test]
async fn get_all() {
    let client = OptionalClient::with_no_credential("http://localhost:3000", None).unwrap();
    let resp = client
        .get_optional_duration_client()
        .get_all(None)
        .await
        .unwrap()
        .into_model()
        .unwrap();
    // According to mockapi.ts, the all endpoint returns { property: "P123DT22H14M12.011S" }
...
async fn put_all() {
    let client = OptionalClient::with_no_credential("http://localhost:3000", None).unwrap();
    // Create a model with property set to duration
    let model = DurationProperty {
    ...
    client
        ...
        .await
        .unwrap();
    // The mockapi expects { property: "P123DT22H14M12.011S" }
}
...
async fn put_default() {
    ...
    let model = DurationProperty::default();
