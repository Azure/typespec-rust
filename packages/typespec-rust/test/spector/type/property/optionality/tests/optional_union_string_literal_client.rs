...
use spector_optionality::{
    models::{UnionStringLiteralProperty, UnionStringLiteralPropertyProperty},
    OptionalClient,
};
...
async fn get_all() {
    ...
    let resp = client
        .get_optional_union_string_literal_client()
        .get_all(None)
        .await
        .unwrap()
        .into_body()
    ...
    assert_eq!(
        resp.property,
        Some(UnionStringLiteralPropertyProperty::World)
    );
}
...
async fn put_all() {
    let client = OptionalClient::with_no_credential("http://localhost:3000", None).unwrap();
    // Create a model with property set to union string literal
    let model = UnionStringLiteralProperty {
        property: Some(UnionStringLiteralPropertyProperty::World),
    };
    ...
    client
        ...
        .await
        .unwrap();
    // The mockapi expects { property: "world" }
}
...
async fn put_default() {
    ...
    let model = UnionStringLiteralProperty::default();
