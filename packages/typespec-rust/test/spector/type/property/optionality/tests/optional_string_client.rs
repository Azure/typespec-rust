...
use spector_optionality::{models::StringProperty, OptionalClient};

#[tokio::test]
async fn get_all() {
    let client = OptionalClient::with_no_credential("http://localhost:3000", None).unwrap();
    let resp = client
        .get_optional_string_client()
        .get_all(None)
        .await
        .unwrap()
        .into_body()
...
async fn put_all() {
    let client = OptionalClient::with_no_credential("http://localhost:3000", None).unwrap();
    // Create a model with property set to string
    let model = StringProperty {
...
async fn put_default() {
    ...
    let model = StringProperty::default();
