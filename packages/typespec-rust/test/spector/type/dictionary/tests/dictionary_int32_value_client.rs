...
async fn get() {
    ...
    let resp = client
        .get_dictionary_int32_value_client()
        .get(None)
        .await
        .unwrap();
    ...
    let mut vec = resp.into_model().unwrap().into_iter().collect::<Vec<_>>();
    vec.sort_by_key(|p| p.0.clone());

    assert_eq!(vec.len(), 2);
    assert_eq!(vec[0].0, "k1");
    assert_eq!(vec[0].1.clone(), 1);
    assert_eq!(vec[1].0, "k2");
    assert_eq!(vec[1].1.clone(), 2);
}

#[tokio::test]
async fn put() {
    let mut body = std::collections::HashMap::new();
    body.insert("k1".to_string(), 1);
    body.insert("k2".to_string(), 2);
