...
async fn create_or_replace() {
    ...
    let resp = client
        ...
        .await
        .unwrap();
    resource = resp.into_model().unwrap();
    assert_eq!(
        resource.etag,
...
async fn export() {
    ...
    let user: User = resp.into_model().unwrap();
    assert_eq!(
        user.etag,
        Some(Etag::from("11bdc430-65e8-45ad-81d9-8ffa60d55b59"))
    );
...
async fn export_all_users() {
    ...
    let user_list: UserList = resp.into_model().unwrap();
...
async fn list_pages() {
    ...
    while let Some(page) = pager.next().await {
        ...
        page_count += 1;
        let paged_user: PagedUser = page.into_model().unwrap();
        ...
        let orders = users[0].orders.as_ref().unwrap();
        assert_eq!(orders.len(), 1);
        assert_eq!(orders[0].detail, Some("a recorder".to_string()));
        assert_eq!(orders[0].id, Some(1));
        assert_eq!(orders[0].user_id, Some(1));
        ...
        let orders = users[1].orders.as_ref().unwrap();
        assert_eq!(orders.len(), 1);
        assert_eq!(orders[0].detail, Some("a TV".to_string()));
        assert_eq!(orders[0].id, Some(2));
        assert_eq!(orders[0].user_id, Some(2));
