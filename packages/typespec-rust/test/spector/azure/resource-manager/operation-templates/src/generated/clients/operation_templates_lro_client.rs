...
use crate::generated::models::{
    ArmOperationStatusResourceProvisioningState, ExportRequest,
            ...
            move |next_link: PollerState<Url>| {
                ...
                async move {
                    let rsp = pipeline
                        ...
                        .await?;
                    let (status, headers, body) = rsp.deconstruct();
                    let final_link = Url::parse(headers.get_str(&HeaderName::from_static(""))?)?;
                    let retry_after = get_retry_after(
                        &headers,
                        &[X_MS_RETRY_AFTER_MS, RETRY_AFTER_MS, RETRY_AFTER],
                        &options.poller_options,
        ...
        Ok(Poller::from_callback(
            move |next_link: PollerState<Url>| {
                ...
                async move {
                    ...
                    let final_link = Url::parse(
                        headers.get_str(&HeaderName::from_static("azure-asyncoperation"))?,
                    )?;
                    let retry_after = get_retry_after(
                        &headers,
                        &[X_MS_RETRY_AFTER_MS, RETRY_AFTER_MS, RETRY_AFTER],
