// Copyright (c) Microsoft Corporation. All rights reserved.
//
// Licensed under the MIT License. See License.txt in the project root for license information.
// Code generated by Microsoft (R) Rust Code Generator. DO NOT EDIT.

use super::{
    ArmOperationStatusResourceProvisioningState, CreateOrReplaceOperationStatus,
    CreateOrUpdateOperationStatus, DeleteOperationStatus, ExtensionsResource,
    ExtensionsResourceListResult, LocationResource, LocationResourceListResult,
    NestedProxyResource, NestedProxyResourceListResult, NotificationDetails,
    SingletonTrackedResource, SingletonTrackedResourceListResult, TopLevelTrackedResource,
    TopLevelTrackedResourceListResult, UpdateOperationStatus,
};
use async_trait::async_trait;
use azure_core::{
    http::{
        pager::Page,
        poller::{PollerStatus, StatusMonitor},
        JsonFormat, RequestContent,
    },
    json::to_json,
    Result,
};

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for ExtensionsResourceListResult {
    type Item = ExtensionsResource;
    type IntoIter = <Vec<ExtensionsResource> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.value.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for LocationResourceListResult {
    type Item = LocationResource;
    type IntoIter = <Vec<LocationResource> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.value.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for NestedProxyResourceListResult {
    type Item = NestedProxyResource;
    type IntoIter = <Vec<NestedProxyResource> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.value.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for SingletonTrackedResourceListResult {
    type Item = SingletonTrackedResource;
    type IntoIter = <Vec<SingletonTrackedResource> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.value.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for TopLevelTrackedResourceListResult {
    type Item = TopLevelTrackedResource;
    type IntoIter = <Vec<TopLevelTrackedResource> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.value.into_iter())
    }
}

impl StatusMonitor for CreateOrReplaceOperationStatus {
    type Output = TopLevelTrackedResource;
    type Format = JsonFormat;
    fn status(&self) -> PollerStatus {
        match &self.status {
            Some(v) => PollerStatus::from(v.as_ref()),
            None => PollerStatus::InProgress,
        }
    }
}

impl StatusMonitor for CreateOrUpdateOperationStatus {
    type Output = SingletonTrackedResource;
    type Format = JsonFormat;
    fn status(&self) -> PollerStatus {
        match &self.status {
            Some(v) => PollerStatus::from(v.as_ref()),
            None => PollerStatus::InProgress,
        }
    }
}

impl StatusMonitor for DeleteOperationStatus {
    type Output = ArmOperationStatusResourceProvisioningState;
    type Format = JsonFormat;
    fn status(&self) -> PollerStatus {
        match &self.status {
            Some(v) => PollerStatus::from(v.as_ref()),
            None => PollerStatus::InProgress,
        }
    }
}

impl StatusMonitor for UpdateOperationStatus {
    type Output = TopLevelTrackedResource;
    type Format = JsonFormat;
    fn status(&self) -> PollerStatus {
        match &self.status {
            Some(v) => PollerStatus::from(v.as_ref()),
            None => PollerStatus::InProgress,
        }
    }
}

impl TryFrom<ExtensionsResource> for RequestContent<ExtensionsResource> {
    type Error = azure_core::Error;
    fn try_from(value: ExtensionsResource) -> Result<Self> {
        Ok(to_json(&value)?.into())
    }
}

impl TryFrom<LocationResource> for RequestContent<LocationResource> {
    type Error = azure_core::Error;
    fn try_from(value: LocationResource) -> Result<Self> {
        Ok(to_json(&value)?.into())
    }
}

impl TryFrom<NestedProxyResource> for RequestContent<NestedProxyResource> {
    type Error = azure_core::Error;
    fn try_from(value: NestedProxyResource) -> Result<Self> {
        Ok(to_json(&value)?.into())
    }
}

impl TryFrom<NotificationDetails> for RequestContent<NotificationDetails> {
    type Error = azure_core::Error;
    fn try_from(value: NotificationDetails) -> Result<Self> {
        Ok(to_json(&value)?.into())
    }
}

impl TryFrom<SingletonTrackedResource> for RequestContent<SingletonTrackedResource> {
    type Error = azure_core::Error;
    fn try_from(value: SingletonTrackedResource) -> Result<Self> {
        Ok(to_json(&value)?.into())
    }
}

impl TryFrom<TopLevelTrackedResource> for RequestContent<TopLevelTrackedResource> {
    type Error = azure_core::Error;
    fn try_from(value: TopLevelTrackedResource) -> Result<Self> {
        Ok(to_json(&value)?.into())
    }
}
