...
use spector_armresources::models::{SingletonTrackedResource, SingletonTrackedResourceProperties};
...
async fn get_by_resource_group() {
    ...
    let resource: SingletonTrackedResource = resp.into_body().await.unwrap();
...
async fn update() {
    ...
    let created_resource: SingletonTrackedResource = resp.into_body().await.unwrap();
    let expected_resource = get_valid_singleton_resource();
