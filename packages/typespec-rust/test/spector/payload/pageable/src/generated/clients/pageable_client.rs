// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
//
// Code generated by Microsoft (R) Rust Code Generator. DO NOT EDIT.

use crate::{
    page_size::clients::PageablePageSizeClient,
    server_driven_pagination::clients::PageableServerDrivenPaginationClient,
    xml_pagination::clients::PageableXmlPaginationClient,
};
use azure_core::{
    fmt::SafeDebug,
    http::{ClientOptions, Pipeline, Url},
    tracing, Result,
};

/// Test for pageable payload.
#[tracing::client]
pub struct PageableClient {
    pub(crate) endpoint: Url,
    pub(crate) pipeline: Pipeline,
}

/// Options used when creating a [`PageableClient`](PageableClient)
#[derive(Clone, Default, SafeDebug)]
pub struct PageableClientOptions {
    /// Allows customization of the client.
    pub client_options: ClientOptions,
}

impl PageableClient {
    /// Creates a new PageableClient requiring no authentication.
    ///
    /// # Arguments
    ///
    /// * `endpoint` - Service host
    /// * `options` - Optional configuration for the client.
    #[tracing::new("Payload.Pageable")]
    pub fn with_no_credential(
        endpoint: &str,
        options: Option<PageableClientOptions>,
    ) -> Result<Self> {
        let options = options.unwrap_or_default();
        let endpoint = Url::parse(endpoint)?;
        if !endpoint.scheme().starts_with("http") {
            return Err(azure_core::Error::with_message(
                azure_core::error::ErrorKind::Other,
                format!("{endpoint} must use http(s)"),
            ));
        }
        Ok(Self {
            endpoint,
            pipeline: Pipeline::new(
                option_env!("CARGO_PKG_NAME"),
                option_env!("CARGO_PKG_VERSION"),
                options.client_options,
                Vec::default(),
                Vec::default(),
                None,
            ),
        })
    }

    /// Returns the Url associated with this client.
    pub fn endpoint(&self) -> &Url {
        &self.endpoint
    }

    /// Returns a new instance of PageablePageSizeClient.
    #[tracing::subclient]
    pub fn get_pageable_page_size_client(&self) -> PageablePageSizeClient {
        PageablePageSizeClient {
            endpoint: self.endpoint.clone(),
            pipeline: self.pipeline.clone(),
        }
    }

    /// Returns a new instance of PageableServerDrivenPaginationClient.
    #[tracing::subclient]
    pub fn get_pageable_server_driven_pagination_client(
        &self,
    ) -> PageableServerDrivenPaginationClient {
        PageableServerDrivenPaginationClient {
            endpoint: self.endpoint.clone(),
            pipeline: self.pipeline.clone(),
        }
    }

    /// Returns a new instance of PageableXmlPaginationClient.
    #[tracing::subclient]
    pub fn get_pageable_xml_pagination_client(&self) -> PageableXmlPaginationClient {
        PageableXmlPaginationClient {
            endpoint: self.endpoint.clone(),
            pipeline: self.pipeline.clone(),
        }
    }
}
