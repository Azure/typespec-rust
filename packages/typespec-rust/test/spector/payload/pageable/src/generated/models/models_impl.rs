// Copyright (c) Microsoft Corporation. All rights reserved.
//
// Licensed under the MIT License. See License.txt in the project root for license information.
// Code generated by Microsoft (R) Rust Code Generator. DO NOT EDIT.

use super::{
    LinkResponse, LinkStringResponse, ListWithoutContinuationResponse, NestedLinkResponse, Pet,
    RequestHeaderNestedResponseBodyResponse, RequestHeaderResponseBodyResponse,
    RequestHeaderResponseHeaderResponse, RequestQueryNestedResponseBodyResponse,
    RequestQueryResponseBodyResponse, RequestQueryResponseHeaderResponse,
};
use async_trait::async_trait;
use azure_core::{http::pager::Page, Result};

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for LinkResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.pets.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for LinkStringResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.pets.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for ListWithoutContinuationResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.pets.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for NestedLinkResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.nested_items.pets.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for RequestHeaderNestedResponseBodyResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.nested_items.pets.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for RequestHeaderResponseBodyResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.pets.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for RequestHeaderResponseHeaderResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.pets.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for RequestQueryNestedResponseBodyResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.nested_items.pets.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for RequestQueryResponseBodyResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.pets.into_iter())
    }
}

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl Page for RequestQueryResponseHeaderResponse {
    type Item = Pet;
    type IntoIter = <Vec<Pet> as IntoIterator>::IntoIter;
    async fn into_items(self) -> Result<Self::IntoIter> {
        Ok(self.pets.into_iter())
    }
}
