// Copyright (c) Microsoft Corporation. All rights reserved.
//
// Licensed under the MIT License. See License.txt in the project root for license information.
// Code generated by Microsoft (R) Rust Code Generator. DO NOT EDIT.

use crate::models::ManagedIdentityTrackedResource;
use async_std::task::block_on;
use azure_core::{RequestContent, Response, Result};
use typespec_client_core::json::to_json;

impl TryFrom<ManagedIdentityTrackedResource> for RequestContent<ManagedIdentityTrackedResource> {
    type Error = azure_core::Error;
    fn try_from(value: ManagedIdentityTrackedResource) -> Result<Self> {
        RequestContent::try_from(to_json(&value)?)
    }
}

impl TryFrom<Response<ManagedIdentityTrackedResource>> for ManagedIdentityTrackedResource {
    type Error = azure_core::Error;
    fn try_from(value: Response<ManagedIdentityTrackedResource>) -> Result<Self> {
        let f = || value.into_json_body();
        let r = block_on(f())?;
        Ok(r)
    }
}
