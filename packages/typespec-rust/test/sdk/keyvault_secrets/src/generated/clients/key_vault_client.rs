...
use crate::models::{
    BackupSecretResult, DeletedSecretBundle, PagedDeletedSecretItem, PagedSecretItem, SecretBundle,
...
use azure_core::{
    ...
    Pager, Pipeline, Policy, Request, RequestContent, Response, Result, Url,
};
use std::sync::Arc;
...
impl KeyVaultClient {
    ...
    pub fn get_deleted_secrets(
    ...
    ) -> Result<Pager<PagedDeletedSecretItem>> {
        ...
        Ok(Pager::from_callback(move |next_link: Option<Url>| {
            ...
            async move {
                let rsp: Response<PagedDeletedSecretItem> =
                    pipeline.send(&mut ctx, &mut request).await?;
                let (status, headers, body) = rsp.deconstruct();
                let bytes = body.collect().await?;
                let res: PagedDeletedSecretItem = json::from_json(bytes.clone())?;
    ...
    pub fn get_secret_versions(
        ...
        Ok(Pager::from_callback(move |next_link: Option<Url>| {
            ...
            match next_link {
                ...
                None => {
                    url = endpoint.clone();
                    let mut path = String::from("/secrets/{secretName}/versions");
                    path = path.replace("{secretName}", &secret_name);
                    url.set_path(&path);
                    url.query_pairs_mut()
                        .append_pair("api-version", &api_version);
    ...
    pub fn get_secrets(
        ...
        Ok(Pager::from_callback(move |next_link: Option<Url>| {
            ...
            let pipeline = pipeline.clone();
            async move {
                let rsp: Response<PagedSecretItem> = pipeline.send(&mut ctx, &mut request).await?;
                let (status, headers, body) = rsp.deconstruct();
                let bytes = body.collect().await?;
                let res: PagedSecretItem = json::from_json(bytes.clone())?;
                let rsp = Response::from_bytes(status, headers, bytes);
                Ok(match res.next_link {
                    Some(next_link) => PagerResult::Continue {
                        response: (rsp),
